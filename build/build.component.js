...
const { noElPrefixFile, toCamel } = require('./common')
...
const runBuild = async () => {
  ...
  const inputs = pkgs
    .map(pkg => pkg.name)
    .filter(name => name.includes('@rol-ui') && !name.includes('utils'))
  ...
  async function build(name) {
    if (!name) return
    const inputOptions = {
      input: path.resolve(__dirname, `../packages/${toCamel(name.split('@rol-ui/')[1])}/index.ts`),
      plugins: [
        nodeResolve(),
        css(),
        vue({
